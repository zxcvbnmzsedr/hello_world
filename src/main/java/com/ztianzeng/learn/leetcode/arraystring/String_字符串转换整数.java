 ...
 *
 * @author zhaotianzeng
 * @date 2019/9/26 1:53 下午
 * @version V1.0
 */
public class String_字符串转换整数 {
    @Test
    public void test() {
    ...
    @Test
    public void test13() {
        Assert.assertEquals(myAtoi("-2147483648"), -2147483648);
    }
    ...
    public int myAtoi(String str) {
        ...
        while (i < chars.length) {
            ...
            if (currentChar == ' ') {
                if (nextNumber && !valid(currentChar)) {
            ...
            if (currentChar == '-' || currentChar == '+') {
                overZ = currentChar == '+';
                nextNumber = true;
            } else if (!valid(currentChar)) {
                return overZ ? result : -result;
            } else {
                nextNumber = true;
                int digit = Character.digit(currentChar, 10);
                if (overZ && (Integer.MAX_VALUE / 10 < result || (Integer.MAX_VALUE / 10 == result && digit > 7))) {
                    return Integer.MAX_VALUE;
                }
                if (!overZ && (Integer.MIN_VALUE / 10 > -result || (Integer.MIN_VALUE / 10 == -result && -digit < -8))) {
