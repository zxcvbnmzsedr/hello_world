...
public class Solution0013 {
    ...
    @Test
    public void test6() {
        Assert.assertEquals(romanToInt("XX"), 20);
    }

    @Test
    public void test7() {
        Assert.assertEquals(romanToInt("LX"), 60);
    }
    ...
    public int romanToInt(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            int curVal = map.get(chars[i]);
            int nextVal = i < s.length() - 1 ? map.get(chars[i + 1]) : 0;
